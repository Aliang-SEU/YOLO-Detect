...
{
    //read_cfg这个函数将所有的参数读到一个图中 参考 http://blog.csdn.net/u014540717/article/details/53193433
    /*首先创建一个list，取名sections，记录一共有多少个section（一个section存储了CNN一层所需参数）；
     * 然后创建一个node，该node的void类型的指针指向一个新创建的section；该section的char类型指针指向.cfg文件中的某一行（line），
     * 然后将该section的list指针指向一个新创建的node，该node的void指针指向一个kvp结构体，
     * kvp结构体中的key就是.cfg文件中的关键字（如：batch，subdivisions等），val就是对应的值*/
    list *sections = read_cfg(filename);
    node *n = sections->front;
    if(!n) error("Config file has no sections");
    network net = make_network(sections->size - 1); //net.n = sections->size - 1
    ...
    parse_net_options(options, &net);   //解析配置文件中的第一段数据
...
list *read_cfg(char *filename)
{
    ...
    char *line;
    int nu = 0;
    list *sections = make_list();   //分配内存并初始化
    section *current = 0;
...
void load_convolutional_weights(layer l, FILE *fp)
{
    ...
    int num = l.n*l.c*l.size*l.size;
    fread(l.biases, sizeof(float), l.n, fp);
    //如果使用了Batch Normalizationn(https://www.zhihu.com/question/38102762, https://arxiv.org/abs/1502.03167)，
    if (l.batch_normalize && (!l.dontloadscales)){
        //加载Batch Normalizatuion的三个参数
        fread(l.scales, sizeof(float), l.n, fp);
        fread(l.rolling_mean, sizeof(float), l.n, fp);
        fread(l.rolling_variance, sizeof(float), l.n, fp);
        if(0){
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    /*size_t fread ( void *buffer, size_t size, size_t count, FILE *stream)
     fread函数从一个文件流中读数据，最多读取count个项，每个项size个字节，如果调用成功返回实
    ...
    int i;
    //cutoff表示之后的参数不加载，fine tuning的时候用
    for(i = start; i < net->n && i < cutoff; ++i){
        //读取每一层的权重
        layer l = net->layers[i];
        if (l.dontload) continue;
        if(l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL){
            load_convolutional_weights(l, fp);  //卷积层的权重系数
        }
        //
        if(l.type == CONNECTED){
