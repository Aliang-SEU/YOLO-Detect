...
#ifndef GPU
    //把所有的x，y，confidence使用LOGISTIC函数映射到（0,1）
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
    ...
    *(l.cost) = 0;
    ...
    for (b = 0; b < l.batch; ++b) {
        ...
        for(t = 0; t < 30; ++t){
            ...
            //计算有 object 框框的 confidence loss
...
void get_region_boxes(layer l, int w, int h, int netw, int neth, float thresh, float **probs, box *boxes, int only_objectness, int *map, float tree_thresh, int relative)
{
    ...
    if (l.batch == 2) { //一次输入两张图片的情况，暂时未使用
        float *flip = l.output + l.outputs;
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w/2; ++i) {
                for (n = 0; n < l.n; ++n) { //每个层数
    ...
    for (i = 0; i < l.w*l.h; ++i){  //每个grid cell 都有一个输出
        int row = i / l.w;
        int col = i % l.w;
        for(n = 0; n < l.n; ++n){   //存储每个grid cell计算之后得到的类别概率 n代表哪一个anchor box
            int index = n*l.w*l.h + i;
            for(j = 0; j < l.classes; ++j){ //每个类别
                probs[index][j] = 0;
            }
            int obj_index = entry_index(l, 0, n*l.w*l.h + i, 4);    //置信度
            int box_index = entry_index(l, 0, n*l.w*l.h + i, 0);    //坐标
